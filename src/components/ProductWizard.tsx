          ...
          <section className="space-y-6">
            ...
            <div className="space-y-4 pt-4">
              <div className="flex items-center justify-between">
                <h4 className="text-[11px] font-black text-text-muted uppercase tracking-[0.2em] px-1">Platform Bazlı Stok Dağılımı</h4>
                <div className="flex items-center space-x-3 pb-2">
                   <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider">Toplu Güncelle:</span>
                   <input 
                      type="number"
                      placeholder="Adet"
                      className="w-20 px-3 py-1.5 text-sm font-bold border border-border-color rounded-xl outline-none focus:ring-4 focus:ring-primary/10"
                      onChange={(e) => {
                        const val = parseInt(e.target.value) || 0;
                        const updatedPlatforms = formData.platforms.map((p: any) => ({ ...p, stock: val }));
                        setFormData((prev: any) => ({ ...prev, platforms: updatedPlatforms }));
                      }}
                   />
                </div>
              </div>
              <div className="card overflow-hidden border-border-color shadow-sm">
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="bg-bg-main text-[10px] uppercase font-bold text-text-muted tracking-widest border-b border-border-color">
                      ...
                      <th className="px-6 py-4">Özel Liste Fiyatı (₺)</th>
                      <th className="px-6 py-4 text-center">Yayın Durumu</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border-color">
                    {formData.platforms.map((p: any, idx: number) => (
                      <tr key={p.name} className="hover:bg-bg-main/50 transition-colors">
                        <td className="px-6 py-4 font-black text-text-main">{p.name}</td>
                        <td className="px-6 py-4">
                          <input 
                            type="number" 
                            value={p.stock}
                            onChange={(e) => handlePlatformChange(idx, 'stock', parseInt(e.target.value))}
                            className="w-24 px-3 py-2 bg-white border border-border-color rounded-xl text-sm font-bold outline-none focus:ring-4 focus:ring-primary/5" 
                          />
                        </td>
                        <td className="px-6 py-4">
                          <input 
                            ...
                            className="w-32 px-3 py-2 bg-white border border-border-color rounded-xl text-sm font-bold outline-none focus:ring-4 focus:ring-primary/5" 
                          />
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex justify-center">
                            <label className="relative inline-flex items-center cursor-pointer">
                              ...
                              />
                              <div className={cn(
                                "w-11 h-6 rounded-full transition-all relative after:content-[''] after:absolute after:top-1 after:left-1 after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all shadow-inner",
